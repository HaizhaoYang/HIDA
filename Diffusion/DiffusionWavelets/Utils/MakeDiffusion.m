...
%
% In:
%    Points    = NxD matrix specifying N points in R^D
...
% (c) Copyright Duke University, 2017, Haizhao Yang
%
...
if ~exist('Metric')
    %[idxs, dists] = nn_search(Points, Points, NN,[],struct('ReturnAsArrays',true));
    [dists,idxs] = knnCPU(Points,Points,NN); % By Haizhao
    idxs=idxs';dists=dists';
else
    %[idxs, dists] = nn_search(Points, Points, NN, Metric,struct('ReturnAsArrays',true)); 
    [dists,idxs] = knnCPU(Points,Points,NN); % By Haizhao
    idxs=idxs';dists=dists';
end
